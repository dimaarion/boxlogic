...
export default function OpenLevelList(props){
    let c = props.class === undefined?"panel":props.class

    const dispatch = useDispatch();
    return <>
        <div className={c} onClick={() => {
            dispatch({type:"OPEN_LEVEL_PANEL",preload:true});
            dispatch({type: "EXIT", preload: false});
        }}>
            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor"
                 className="bi bi-grid-3x3-gap" viewBox="0 0 16 16">
                <path
